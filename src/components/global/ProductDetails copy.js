...
const ProductDetails = () => {
  ...
  // Helper to support both formats for productName
  ...
  const handleAddToCart = async () => {
    ...
    if (!selectedColor || !selectedSize || !selectedLocation) {
      toast.warn("Please choose size, color, and location.");
      return;
    }
    ...
    const cartItem = {
      cartId,
      productId: id,
      name: getLocalizedProductName(), // 🟢 Use localized product name
    ...
    <div className='max-w-screen-xl mx-auto p-4 text-gray-800'>
      <div className='grid grid-cols-1 lg:grid-cols-2 gap-6'>
        ...
        <div>
          ...
          >
            <SwiperSlide>
              <img
                ...
                alt='Main Product'
                className='object-contain w-full h-96 border rounded'
              />
            </SwiperSlide>
          ...
          {/* Thumbnails - Only show if additional images exist */}
          {product.additionalImageUrls?.length > 0 && (
            <Swiper
              modules={[Thumbs]}
              onSwiper={setThumbsSwiper}
              spaceBetween={10}
              slidesPerView={Math.min(4, product.additionalImageUrls.length)}
              watchSlidesProgress
              className='px-2 mt-4'
            >
              {product.additionalImageUrls.map((img, idx) => (
                <SwiperSlide key={idx}>
                  <img
                    src={img}
                    alt={`Thumb ${idx}`}
                    className='h-20 w-24 object-cover border rounded cursor-pointer hover:opacity-80 transition'
                  />
                </SwiperSlide>
              ))}
            </Swiper>
