...
import { defaultCountryCodeOptions } from "../constants/productOptions";
...
const BecomeSupplierForm = () => {
    ...
    const fetchCategories = async () => {
      try {
        ...
        querySnapshot.forEach((doc) => {
          const data = doc.data();
          if (data.category) {
            categorySet.add(data.category); // 👈 Collect unique categories
          }
        });
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setFormData({ ...initialValues }); // Optional: Reset form
