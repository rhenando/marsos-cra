...
const ConstructionSubcategories = () => {
  ...
  const [constructionSubcategories, setConstructionSubcategories] = useState(
    []
  );
  ...
  const currencySymbol = locale === "ar" ? "ر.س." : "$";
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        ...
        const constructionProducts = productList.filter(
          (product) => product.category === "Construction"
        );

        const constructionSubcategoriesSet = new Set(
        ...
        setConstructionSubcategories([
          ...Array.from(constructionSubcategoriesSet),
        ]);
  ...
  const sliderSettings = {
    infinite: constructionSubcategories.length > 5,
    speed: 500,
    slidesToShow: Math.min(5, constructionSubcategories.length),
    slidesToScroll: 1,
    ...
    responsive: [
      {
        breakpoint: 1024,
        settings: {
          slidesToShow: Math.min(4, constructionSubcategories.length),
        },
      },
      {
        breakpoint: 768,
        settings: {
          slidesToShow: Math.min(3, constructionSubcategories.length),
        },
      },
      {
        breakpoint: 576,
        settings: {
          slidesToShow: Math.min(2, constructionSubcategories.length),
        },
      },
      {
        breakpoint: 375,
        settings: {
          slidesToShow: Math.min(1, constructionSubcategories.length),
        },
      },
    ],
  };
    ...
    <div dir={locale === "ar" ? "rtl" : "ltr"}>
      <div className='container my-5'>
        ...
        <Slider {...sliderSettings}>
          {constructionSubcategories.map((subcategory, index) => {
