...
const EquipmentSubcategories = () => {
  ...
  const [equipmentSubcategories, setEquipmentSubcategories] = useState([]);
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        ...
        const equipmentProducts = productList.filter(
          (product) => product.category === "Equipment"
        );

        const equipmentSubcategoriesSet = new Set(
          equipmentProducts
        ...
        setEquipmentSubcategories([...Array.from(equipmentSubcategoriesSet)]);
        setFilteredProducts(equipmentProducts);
        setLoading(false);
      } catch (error) {
  ...
  const sliderSettings = {
    infinite: equipmentSubcategories.length > 5,
    speed: 500,
    slidesToShow: Math.min(5, equipmentSubcategories.length),
    slidesToScroll: 1,
    ...
    responsive: [
      {
        breakpoint: 1024,
        settings: {
          slidesToShow: Math.min(4, equipmentSubcategories.length),
        },
      },
      {
        breakpoint: 768,
        settings: {
          slidesToShow: Math.min(3, equipmentSubcategories.length),
        },
      },
      {
        breakpoint: 576,
        settings: {
          slidesToShow: Math.min(2, equipmentSubcategories.length),
        },
      },
      {
        breakpoint: 375,
        settings: {
          slidesToShow: Math.min(1, equipmentSubcategories.length),
        },
      },
    ],
  };
...
export default EquipmentSubcategories;
