...
const SaudiManufacturedProductsSubcategories = () => {
  const [products, setProducts] = useState([]);
  const [loading, setLoading] = useState(true);
  const [saudiSubcategories, setSaudiSubcategories] = useState([]);
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        ...
        const saudiProducts = productList.filter(
          (product) => product.category === "Saudi Manufactured Products"
        );

        const saudiSubcategoriesSet = new Set(
          saudiProducts.map((product) => product.subCategory).filter(Boolean)
        );

        setProducts(saudiProducts);
        setSaudiSubcategories([...Array.from(saudiSubcategoriesSet)]);
        setFilteredProducts(saudiProducts);
        setLoading(false);
      } catch (error) {
  ...
  const sliderSettings = {
    // dots: saudiSubcategories.length > 1,
    infinite: saudiSubcategories.length > 5,
    speed: 500,
    slidesToShow: Math.min(5, saudiSubcategories.length),
    slidesToScroll: 1,
    ...
    responsive: [
      {
        breakpoint: 1024,
        settings: {
          slidesToShow: Math.min(4, saudiSubcategories.length),
        },
      },
      {
        breakpoint: 768,
        settings: {
          slidesToShow: Math.min(3, saudiSubcategories.length),
        },
      },
      {
        breakpoint: 576,
        settings: {
          slidesToShow: Math.min(2, saudiSubcategories.length),
        },
      },
      {
        breakpoint: 375,
        settings: {
          slidesToShow: Math.min(1, saudiSubcategories.length),
        },
      },
    ],
  };
    ...
    <div dir={locale === "ar" ? "rtl" : "ltr"}>
      <div className='container my-5'>
        ...
        <Slider {...sliderSettings}>
          {saudiSubcategories.map((subcategory, index) => {
      ...
      <div className='container my-5'>
        <div className='d-flex justify-content-between align-items-center flex-wrap mb-4'>
          <div className='btn-group flex-wrap mb-2'>
            {saudiSubcategories.map((subcategory) => (
              <button
                key={subcategory}
        ...
        <div className='row row-cols-1 row-cols-sm-2 row-cols-md-3 row-cols-lg-5 g-4'>
          {filteredProducts.map((product) => {
            ...
            const origin = product.mainLocation || t("Unknown Origin"); // Default to "Unknown Origin" if not provided
...
export default SaudiManufacturedProductsSubcategories;
